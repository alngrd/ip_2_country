...
func TestLoad_Defaults(t *testing.T) {
	...
	if cfg.DatabaseURL != "csv:data/ip2country.csv" {
		t.Errorf("DatabaseURL: expected csv:data/ip2country.csv, got %s", cfg.DatabaseURL)
	}
}
...
func TestLoad_DatabaseURL_DefaultWhenEmpty(t *testing.T) {
	t.Setenv("DATABASE_URL", "")

	cfg, err := config.Load()
	if err != nil {
		t.Fatalf("unexpected error with empty DATABASE_URL: %v", err)
	}
	if cfg.DatabaseURL != "csv:data/ip2country.csv" {
		t.Errorf("DatabaseURL: expected default csv:data/ip2country.csv, got %s", cfg.DatabaseURL)
	}
}

func TestLoad_DatabaseURL_ExplicitValueOverridesDefault(t *testing.T) {
	t.Setenv("DATABASE_URL", "csv:/tmp/custom.csv")
